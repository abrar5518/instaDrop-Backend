...
class WhatsAppService
{
    ...
    {
        $this->adminNumber = (string) config('services.whatsapp.admin_number');
        $this->apiToken = config('services.whatsapp.access_token');
        $this->phoneNumberId = config('services.whatsapp.phone_number_id');
        $this->graphVersion = config('services.whatsapp.graph_version', 'v25.0');
    }

    protected function adminNumber(): string
    {
        try {
            return SystemSetting::query()->value('admin_whatsapp_number') ?: $this->adminNumber;
        } catch (\Throwable) {
