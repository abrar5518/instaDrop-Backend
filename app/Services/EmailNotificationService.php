...
class EmailNotificationService
{
    ...
    /** Send the dispatch mailbox a copy of the newly issued payment link. */
    public function sendAdminInvoiceCreatedEmail(Invoice $invoice): bool
    {
        ...
        if (!$admin || !$order) {
            Log::warning('Admin invoice email skipped because admin email or order is missing.', ['invoice' => $invoice->invoice_number]);
            return false;
        }
        ...
        return $this->send($admin, "Payment link issued: {$invoice->invoice_number} - {$order->customer_name}", "A payment link was issued for {$order->customer_name}.\n\nPay securely: {$paymentUrl}", [
            'eyebrow' => 'Admin · Payment link issued',
            'title' => 'A customer payment link is ready.',
            'intro' => 'The quotation was converted into an invoice and the customer payment email was sent.',
            'amount' => 'GBP ' . number_format((float) $invoice->total_amount, 2),
            ...
            'details' => [
                ...
                'Collection' => $order->pickup_address,
                'Delivery' => $order->delivery_address,
            ],
            'actionLabel' => 'Open customer payment page',
            'actionUrl' => $paymentUrl,
            'note' => 'This is the same secure payment link sent to the customer.',
        ]);
    }
    ...
    private function send(string $to, string $subject, string $body, array $presentation = []): bool
    {
        ...
        try {
            ...
            $data = array_merge([
                ...
                'plainBody' => $body,
                'businessName' => $setting?->business_name ?: 'InstaDrop Courier Services',
