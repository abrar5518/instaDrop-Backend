...
<body class="bg-[#f8fafc] text-slate-800 min-h-screen flex">
    ...
    <aside class="w-64 bg-[#0a192f] border-r border-slate-800 flex flex-col justify-between p-6 shrink-0 shadow-xl">
        <div class="space-y-8">
            ...
            <nav class="space-y-1.5 text-xs font-semibold">
                ...
                <a href="{{ route('admin.inquiries.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-white/10 text-slate-200 hover:text-white transition-all">
                    <svg class="w-4 h-4 text-[#c6ff00]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M21 15a4 4 0 0 1-4 4H8l-5 3V7a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4z"/></svg>
                    <span>Contact & Business Leads</span>
                </a>
                <a href="{{ route('admin.blogs.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-white/10 text-slate-200 hover:text-white {{ request()->routeIs('admin.blogs.*') ? 'bg-white/10' : '' }}"><span class="text-[#c6ff00]">▤</span><span>Blogs & SEO</span></a>
                <a href="{{ route('admin.services.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-white/10 text-slate-200 hover:text-white {{ request()->routeIs('admin.services.*') ? 'bg-white/10' : '' }}"><span class="text-[#c6ff00]">▦</span><span>Services</span></a>
                <a href="{{ route('admin.coverage.edit') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-white/10 text-slate-200 hover:text-white {{ request()->routeIs('admin.coverage.*') ? 'bg-white/10' : '' }}"><span class="text-[#c6ff00]">⌖</span><span>Coverage</span></a>
                ...
                <a href="{{ route('admin.security.edit') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-white/10 text-slate-200 hover:text-white transition-all">
                    <svg class="w-4 h-4 text-[#c6ff00]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="10" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
    ...
    <main class="flex-1 min-w-0 overflow-y-auto p-8 space-y-6">
        @if (session('success'))
